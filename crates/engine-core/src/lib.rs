...
"#,
    ...
    #[test]
    fn emit_go_runs_aot_object_create_prototype_subset() {
        let root = temp_project("engine-core-aot-object-create-prototype");
        write(
            &root,
            "src/index.js",
            r#"
const proto = { inherited: "yes", shadowed: "proto" }
const child = Object.create(proto)
child.own = "ok"
child.shadowed = "own"
console.log("object-create-proto", child.own, child.inherited, child.shadowed, "inherited" in child, Object.hasOwn(child, "inherited"), Object.keys(child).join("|"), JSON.stringify(child), child.__tsgodownPrototype === undefined, "__tsgodownPrototype" in child, Object.hasOwn(child, "__tsgodownPrototype"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-object-create-prototype".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(response.files[0].contents.contains("tsgodownObjectCreate"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownObjectHasProperty"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "object-create-proto ok yes own true false own|shadowed {\"own\":\"ok\",\"shadowed\":\"own\"} true false false\n"
        );
    }
...
"#,
    ...
    #[test]
    fn emit_go_runs_aot_string_split_join_subset() {
        let root = temp_project("engine-core-aot-string-split-join");
        write(
            &root,
            "src/index.js",
            r#"
...
"#,
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-string-split-join".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "string-split alpha|beta|gamma a\\s{0,2}b\\s{0,2}\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_for_of_pair_destructuring_subset() {
        let root = temp_project("engine-core-aot-for-of-pair-destructuring");
        write(
            &root,
            "src/index.js",
            r#"
...
console.log("pairs", makeSafeRegex("\\d+\\s*[a-zA-Z0-9-]*"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-for-of-pair-destructuring".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "pairs \\d{1,256}\\s{0,1}[a-zA-Z0-9-]{0,250}\n"
        );
    }
...
"#,
    ...
    #[test]
    fn emit_go_runs_aot_uint8array_set_and_length_subset() {
        let root = temp_project("engine-core-aot-uint8array-set-and-length");
        write(
            &root,
            "src/index.js",
            r#"
const head = Uint8Array.of(1, 2, 3)
const tail = Uint8Array.of(4, 5)
const bytes = new Uint8Array(head.length + tail.length)
bytes.set(head)
bytes.set(tail, head.length)
console.log("bytes-set", bytes.length, bytes[0], bytes[3], bytes[4])
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-uint8array-set-and-length".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(response.files[0].contents.contains("copy("));
...
"#,
    ...
    #[test]
    fn emit_go_runs_aot_prototype_call_alias_subset() {
        let root = temp_project("engine-core-aot-prototype-call-alias");
        write(
            &root,
            "src/index.js",
            r#"
...
const values = [
  ...
  upper.call("ab"),
  trim.call("  x  "),
  slice.call("abcdef", 1, 4),
  indexOf.call("abcabc", "bc", 2),
  includes.call("abcdef", "cd"),
  charAt.call("abc", 1),
  charCodeAt.call("ABC", 1),
  test.call(/^a/, "abc"),
  tag.call(["x"])
]
console.log("prototype-call", values.join("|"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-prototype-call-alias".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(!response.files[0].contents.contains("var lower"));
        assert!(response.files[0].contents.contains("regexp.MustCompile"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "prototype-call ab|AB|x|bcd|4|true|b|66|true|[object Array]\n"
        );
    }
...
"#,
    ...
    #[test]
    fn emit_go_runs_aot_array_from_length_and_fill_subset() {
        let root = temp_project("engine-core-aot-array-from-fill");
        write(
            &root,
            "src/index.js",
            r#"
const filled = Array.from({ length: 4 }).fill("x", 1, 3)
const sparse = new Array(3)
sparse[1] = "mid"
function normalize(values, value) {
  return values.fill(value, -2).join("|")
}
console.log("array-from-fill", filled.join(","), sparse.join(","), normalize(Array.from({ length: 3 }), "z"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-from-fill".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(response.files[0]
            .contents
            .contains("tsgodownAnyArrayWithLength"));
        assert!(response.files[0].contents.contains("tsgodownAnyArrayFill"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-from-fill ,x,x, ,mid, |z|z\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_array_from_length_map_subset() {
        let root = temp_project("engine-core-aot-array-from-map");
        write(
            &root,
            "src/index.js",
            r#"
const values = ["zero", "one", "two"]
const copied = Array.from({ length: 3 }, (_, index) => values[index])
console.log("array-from-map", copied.join("|"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-from-map".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(response.files[0]
            .contents
            .contains("tsgodownAnyArrayFromLengthMap"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-from-map zero|one|two\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_array_map_subset() {
        let root = temp_project("engine-core-aot-array-map");
        write(
            &root,
            "src/index.js",
            r#"
...
const copiedWords = words.map((value, index) => value)
const indexes = mixed.map((value, index) => index)
console.log("array-map", copiedWords.join("|"), indexes.join("|"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-map".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(response.files[0]
            .contents
            .contains("tsgodownStringArrayMap"));
        assert!(response.files[0].contents.contains("tsgodownAnyArrayMap"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-map zero|one|two 0|1|2\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_array_map_destructuring_prelude_subset() {
        let root = temp_project("engine-core-aot-array-map-destructuring-prelude");
        write(
            &root,
            "src/index.js",
            r#"
const matrix = [
  ["index.js", "*.js", { dot: false }],
  [".env", "*", { dot: true }],
]
const report = {
  package: "fixture",
...
console.log("array-map-destructure", JSON.stringify(report))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-map-destructuring-prelude".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-map-destructure {\"package\":\"fixture\",\"probes\":[{\"dot\":false,\"path\":\"index.js\",\"pattern\":\"*.js\",\"same\":false},{\"dot\":true,\"path\":\".env\",\"pattern\":\"*\",\"same\":false}]}\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_logical_value_expression_subset() {
        let root = temp_project("engine-core-aot-logical-value-expression");
        write(
            &root,
            "src/index.js",
            r#"
const truthy = true && [2, 5]
const falsy = false && [2, 5]
const fallback = "" || { label: "fallback" }
const report = [
  truthy && {
    start: truthy[0],
    end: truthy[1],
    body: "abcdef".slice(truthy[0], truthy[1]),
  },
  falsy && { unreachable: true },
  fallback,
]
console.log("logical-value", JSON.stringify(report))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-logical-value-expression".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "logical-value [{\"body\":\"cde\",\"end\":5,\"start\":2},false,{\"label\":\"fallback\"}]\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_array_predicate_callbacks_subset() {
        let root = temp_project("engine-core-aot-array-predicate-callbacks");
        write(
            &root,
            "src/index.js",
            r#"
const stdio = ["pipe", "ignore", "pipe"]
const mixed = [0, "ready", false]
const hasReady = mixed.some((value, index) => value === "ready" && index === 1)
const allPipeish = stdio.every((value, index) => value === "pipe" || index === 1)
const none = [].some(() => true)
const emptyEvery = [].every(() => false)
console.log("array-predicate", hasReady, allPipeish, none, emptyEvery)
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-predicate-callbacks".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(response.files[0].contents.contains("tsgodownAnyArraySome"));
        assert!(response.files[0].contents.contains("tsgodownAnyArrayEvery"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownStringArrayEvery"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-predicate true true false true\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_array_filter_callbacks_subset() {
        let root = temp_project("engine-core-aot-array-filter-callbacks");
        write(
            &root,
            "src/index.js",
            r#"
const raw = ["", "alpha", "beta", ""]
const compact = raw.filter(Boolean)
const selected = raw.filter((value, index) => value !== "" && index < 3)
const mixed = [0, "go", false, "node"].filter(Boolean)
console.log("array-filter", compact.join("|"), selected.join("|"), mixed.join("|"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-filter-callbacks".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(response.files[0]
            .contents
            .contains("tsgodownStringArrayFilter"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownAnyArrayFilter"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-filter alpha|beta alpha|beta go|node\n"
        );
    }
...
"#,
    ...
    #[test]
    fn emit_go_runs_aot_array_reduce_callbacks_subset() {
        let root = temp_project("engine-core-aot-array-reduce-callbacks");
        write(
            &root,
            "src/index.js",
            r#"
...
const total = [1, 2, 3].reduce((acc, value, index) => acc + value + index, 0)
console.log("array-reduce", JSON.stringify(object), total)
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-reduce-callbacks".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(response.files[0]
            .contents
            .contains("tsgodownStringArrayReduce"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownAnyArrayReduce"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-reduce {\"alpha\":1,\"beta\":2} 9\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_array_for_each_callbacks_subset() {
        let root = temp_project("engine-core-aot-array-for-each-callbacks");
        write(
            &root,
            "src/index.js",
            r#"
const out = {}
const seen = []
Object.keys({ alpha: 1, beta: 2 }).forEach((key, index) => {
  out[key] = index + 1
  seen.push(key)
});
["skip", "", "keep"].filter(Boolean).forEach((value) => {
  seen.push(value)
})
console.log("array-for-each", JSON.stringify(out), seen.join("|"))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-array-for-each-callbacks".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(response.files[0].contents.contains("for __tsgodownIndex"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownStringArrayFilter"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "array-for-each {\"alpha\":1,\"beta\":2} alpha|beta|skip|keep\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_object_entries_for_each_subset() {
        let root = temp_project("engine-core-aot-object-entries-for-each");
        write(
            &root,
            "src/index.js",
            r#"
const out = {}
Object.entries({ beta: 2, alpha: 1 }).forEach(([key, value]) => {
  out[key] = value
})
console.log("object-entries", JSON.stringify(out))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-object-entries-for-each".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(response.files[0].contents.contains("tsgodownObjectEntries"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownAnyArrayFromAny"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "object-entries {\"beta\":2,\"alpha\":1}\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_ts_string_enum_iife_subset() {
        let root = temp_project("engine-core-aot-ts-string-enum-iife");
        write(
            &root,
            "src/index.js",
            r#"
...
console.log("ts-enum", DefaultValuesForTypeKey.BOOLEAN, DefaultValuesForTypeKey.ARRAY, JSON.stringify(DefaultValuesForTypeKey))
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-ts-string-enum-iife".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "ts-enum boolean array {\"BOOLEAN\":\"boolean\",\"STRING\":\"string\",\"NUMBER\":\"number\",\"ARRAY\":\"array\"}\n"
        );
    }
    ...
    fn emit_go_runs_aot_math_numeric_subset() {
        ...
        write(
            ...
            r#"
const values = [2, 5, -3]
const high = Math.max(values.length, 4, Math.floor(4.8))
const low = Math.min(-1, Math.ceil(-1.2), Math.trunc(2.9))
const rounded = Math.round(1.5)
const powered = Math.pow(2, 3)
console.log("math-numeric", high, low, rounded, powered)
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-math-numeric".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(response.files[0].contents.contains("tsgodownMathMax"));
        assert!(response.files[0].contents.contains("tsgodownMathMin"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "math-numeric 4 -1 2 8\n"
        );
    }

    #[test]
    fn emit_go_runs_aot_date_iso_subset() {
        let root = temp_project("engine-core-aot-date-iso");
        write(
            &root,
            "src/index.js",
            r#"
...
const utc = new Date(Date.UTC(2026, 4, 15, 1, 2, 3, 4))
console.log("date-iso", epoch.toISOString(), toISO.call(utc), epoch instanceof Date, "x" instanceof Date)
"#,
        );
        ...
        let response = emit_go(EmitGoRequest {
            ...
            package_name: None,
            module_path: Some("example.com/aot-date-iso".to_string()),
            output_kind: EmitGoOutputKind::Main,
            ir_snapshot: None,
        });
        ...
        assert!(!response.files[0].contents.contains("tsgodownrt.RunProgram"));
        assert!(!response.files[0].contents.contains("var toISO"));
        assert!(response.files[0]
            .contents
            .contains("tsgodownDateUTCISOString"));
        ...
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "date-iso 1970-01-01T00:00:00.000Z 2026-05-15T01:02:03.004Z true false\n"
        );
    }
