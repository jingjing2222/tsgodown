...
test("M1 regression: mixed relative sourceRoot forms across JS+d.ts maps in nested package dirs canonicalize to one typed source and keep Go path green", async () => {
  const cwd = fs.mkdtempSync(
    path.join(
      os.tmpdir(),
      "tsgodown-pipeline-mixed-relative-sourceroot-nested-pkg-e2e-",
    ),
  );
  tempDirs.push(cwd);

  fs.mkdirSync(path.join(cwd, "dist", "maps"), { recursive: true });
  fs.mkdirSync(path.join(cwd, "packages", "api", "src", "routes"), {
    recursive: true,
  });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      version: 3,
      file: "../index.mjs",
      sourceRoot: "../.././packages/api/src/nested/..",
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      version: 3,
      file: "../index.d.ts",
      sourceRoot: "../../packages/./api/src/routes",
      sources: ["../routes/health.ts%3Ffrom%3Dtypes%23decl"],
      names: [],
      mappings: "",
    }),
