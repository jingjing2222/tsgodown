...
| aot.module.registry | Module registry | WIP | WIP | AOT_MODULE_REGISTRY_UNSUPPORTED | focused unit tests | Local ESM named function and primitive value imports plus simple CJS `module.exports = functionName` default `require` imports can bind to direct Go declarations without `RunProgram`; CJS object exports, default interop, cache records pending. |
