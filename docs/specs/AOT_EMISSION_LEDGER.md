...
| aot.expr.string | String expressions | WIP | WIP | AOT_STRING_EXPR_UNSUPPORTED | focused unit tests | String literals, string parameter inference from concatenation/method calls, string `+`, string `+=`, typed conditional expressions, `typeof`, `.trim`, and `.toLowerCase` for local slots emit typed Go strings. Full JS string object/method/coercion semantics pending. |
| aot.expr.boolean | Boolean expressions | WIP | WIP | AOT_BOOLEAN_EXPR_UNSUPPORTED | focused unit tests | Boolean literals, numeric/string/bool/`typeof` comparison predicates, logical-not, logical-and, logical-or, typed conditional expressions, and string `.includes` for bool subset. Truthiness pending. |
