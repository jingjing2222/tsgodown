...
| aot.module.registry | Module registry | WIP | WIP | AOT_MODULE_REGISTRY_UNSUPPORTED | focused unit tests | Local ESM named function, class, and primitive value imports plus simple CJS `module.exports = functionName` default `require` imports, `exports.name = functionName` namespace calls, and object namespace exports such as `const api = { fn }; module.exports = api` can bind function declarations and top-level function-expression bindings to direct Go declarations without `RunProgram`; CJS general object values, default interop, cache records pending. |
