...
| aot.value.model | Typed Value model | WIP | WIP | AOT_VALUE_MODEL_UNSUPPORTED | focused unit tests | Numeric slots emit typed Go `float64`, string slots emit typed Go `string`, and boolean slots emit typed Go `bool`; full backend-neutral JS value contract still pending. |
