...
| aot.module.registry | Module registry | WIP | WIP | AOT_MODULE_REGISTRY_UNSUPPORTED | focused unit tests | Local ESM named function, class, and primitive value imports plus simple CJS `module.exports = functionName` default `require` imports and `exports.name = functionName` namespace calls can bind function declarations and top-level function-expression bindings to direct Go declarations without `RunProgram`; CJS object values, default interop, cache records pending. |
