...
| aot.function.decl | Function declarations/classes | WIP | WIP | AOT_FUNCTION_DECL_UNSUPPORTED | focused unit tests | Simple numeric function declarations and top-level `const fn = (...) => { return ... }` / `const fn = function () { return ... }` bindings emit direct Go functions; basic constructor field assignment classes emit Go structs, constructors, and methods. Closures, full `this`, rest, async, generator pending. |
...
| aot.control.loops | Native loop control flow | WIP | WIP | AOT_LOOP_UNSUPPORTED | focused unit tests | Simple top-level and function-local numeric `for` loops emit native Go control flow. `while`, `for-of`, labels, and iterator closing pending. |
