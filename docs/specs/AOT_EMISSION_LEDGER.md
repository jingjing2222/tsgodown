...
| aot.module.registry | Module registry | WIP | WIP | AOT_MODULE_REGISTRY_UNSUPPORTED | focused unit tests | Local ESM named function and primitive value imports plus simple CJS `module.exports = functionName` default `require` imports and `exports.name = functionName` namespace calls can bind to direct Go declarations without `RunProgram`; CJS object values, default interop, cache records pending. |
...
| aot.function.decl | Function declarations/classes | WIP | WIP | AOT_FUNCTION_DECL_UNSUPPORTED | focused unit tests | Simple numeric functions emit direct Go functions; basic constructor field assignment classes emit Go structs, constructors, and methods. Closures, full `this`, rest, async, generator pending. |
