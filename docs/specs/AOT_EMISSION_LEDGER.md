...
| aot.module.registry | Module registry | TODO | TODO | AOT_MODULE_REGISTRY_UNSUPPORTED | planned | Need backend-neutral module records and direct Go package/module registry. |
...
| aot.control.if_return | Native if and return control flow | WIP | WIP | AOT_CONTROL_IF_RETURN_UNSUPPORTED | focused unit tests | Function-local `if` plus `return` emits native Go control flow for numeric predicates. |
...
| aot.value.model | Typed Value model | TODO | TODO | AOT_VALUE_MODEL_UNSUPPORTED | planned | Need backend-neutral JS value contract rendered as typed Go values, not `map[string]any` interpreter state. |
...
| aot.no_ir_json_interpreter | No IR JSON interpreter in generated Go | WIP | WIP | AOT_IR_JSON_INTERPRETER_FORBIDDEN | focused unit tests | Simple console/function/control-flow cases assert no `RunProgram`; global gate pending. |
